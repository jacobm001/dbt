import itertools
import os
from typing import Iterable, Dict, Any, Union, List, Optional, Tuple
...
class SchemaParser(SimpleParser[SchemaTestBlock, ParsedTestNode]):
    ...
    def get_fqn(self, path: str, name: str, package_name: str) -> List[str]:
        fqn = super().get_fqn(path, name)
        fqn[0] = package_name
        return fqn
    ...
    def parse_node(self, block: SchemaTestBlock) -> ParsedTestNode:
        ...
        fqn = self.get_fqn(
            fqn_path, builder.fqn_name, builder.package_name
        )
