...
from dbt.contracts.graph.parsed import ParsedNode, HasUniqueID
...
# FileBlockType = TypeVar('FileBlockType', bound=FileBlock)
FileBlockType = FileBlock

IntermediateNode = TypeVar('IntermediateNode', bound=Any)
FinalNode = TypeVar('FinalNode', bound=ParsedNode)
...
class BaseParser(Generic[FinalValue]):
    ...
    @abc.abstractmethod
    def parse_file(self, block: FileBlockType) -> None:
        pass
...
class ConfiguredParser(
    ...
    def add_result_node(self, block: FileBlock, node: ParsedNode):
